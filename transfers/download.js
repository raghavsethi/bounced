    ...
    User.find({ 'ip': req.ip }, function (error, users) {
        ...
        if (requestType.toLowerCase() == "bounced") {
            ...
            Friendship.find({ $or: [{ 'friend1': requestDownloader }, { 'friend2': requestDownloader}] }, {}, { limit: 5, sort: [['count', 'desc']] }, function (error, friends) {
                ...
                Pending.find({ 'fileHash': newPending.fileHash, 'downloader':requestDownloader, 'uploader': newPending.uploader, 'type': newPending.type }, function (error, pendingUsers) {
                    if (error == null) {
                        ...
                        logger.warn('download.js-downloadHandler: Could not read pendings from DB', error);
                        res.send({ 'status': 'Error', 'text': error });
                        return;
                    }
                });
            ...
            Pending.find({ 'fileHash': newPending.fileHash, 'downloader':requestDownloader, 'uploader': newPending.uploader, 'type': newPending.type }, function (error, pendingUsers) {
                ...
                if (pendingUsers.length == 0) {
                    ...
                    logger.info('download.js-downloadHandler: Added pending to  ' + users[0].nick + 
                    ' with type = direct and ' + bounced.length + ' pending(s) with type = firstleg ');
                    var downloadLog = {};
                    downloadLog["type"] = "download";
                    downloadLog["transferID"] = newPending.transferID;
                    downloadLog["downloader"] = newPending.downloader;
                    downloadLog["uploader"] = newPending.uploader;
                    downloadLog["fileSize"] = newPending.fileSize;
                    researchLogger.info(downloadLog);
                    res.send({ 'status': 'OK', 'text': 'Download request accepted' });
